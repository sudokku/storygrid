...
const twoLeafContainer: GridNode = {
  ...
  sizes: [1, 1],
  children: [
    { type: 'leaf', id: 'child-1', mediaId: null, fit: 'cover', objectPosition: 'center center', backgroundColor: null, audioEnabled: true, hasAudioTrack: true },
    { type: 'leaf', id: 'child-2', mediaId: null, fit: 'cover', objectPosition: 'center center', backgroundColor: null, audioEnabled: true, hasAudioTrack: true },
  ],
};
...
describe('Sidebar', () => {
  ...
  describe('Audio locked state in sidebar (MUTE-03)', () => {
    const noAudioVideoLeaf: GridNode = {
      type: 'leaf',
      id: 'leaf-noaudio',
      mediaId: 'vid-2',
      fit: 'cover',
    ...
    beforeEach(() => {
      useGridStore.setState({
        ...
        mediaRegistry: { 'vid-2': 'blob:video2' },
        mediaTypeMap: { 'vid-2': 'video' },
      });
      useEditorStore.setState({ selectedNodeId: 'leaf-noaudio' });
    });

    it('renders disabled sidebar audio button when video has no audio track', () => {
      render(<Sidebar />);
      expect(screen.getByTestId('sidebar-audio-button')).toBeDisabled();
    });

    it('locked sidebar button has aria-label "No audio track"', () => {
      render(<Sidebar />);
      expect(screen.getByTestId('sidebar-audio-button')).toHaveAttribute('aria-label', 'No audio track');
    });

    it('shows "No audio track" label text when video has no audio', () => {
      render(<Sidebar />);
      expect(screen.getByText('No audio track')).toBeInTheDocument();
    });

    it('locked sidebar button does not call toggleAudioEnabled on click', () => {
      const toggleAudioEnabled = vi.fn();
      useGridStore.setState({ toggleAudioEnabled });
      render(<Sidebar />);
      fireEvent.click(screen.getByTestId('sidebar-audio-button'));
    ...
    it('renders interactive toggle when video HAS audio track', () => {
      ...
      useGridStore.setState({
        root: withAudio,
        mediaRegistry: { 'vid-3': 'blob:video3' },
        mediaTypeMap: { 'vid-3': 'video' },
      });
      useEditorStore.setState({ selectedNodeId: 'leaf-withaudio' });
      render(<Sidebar />);
      expect(screen.getByTestId('sidebar-audio-button')).not.toBeDisabled();
      expect(screen.getByText('Cell audio')).toBeInTheDocument();
    });
  });
  ...
  describe('Cell dimensions', () => {
    ...
    it('shows correct dimensions for a leaf in a horizontal container (50% width)', () => {
      const hContainer: GridNode = {
        ...
        sizes: [1, 1],
        children: [
          { type: 'leaf', id: 'h-child-1', mediaId: null, fit: 'cover', objectPosition: 'center center', backgroundColor: null, audioEnabled: true, hasAudioTrack: true },
          { type: 'leaf', id: 'h-child-2', mediaId: null, fit: 'cover', objectPosition: 'center center', backgroundColor: null, audioEnabled: true, hasAudioTrack: true },
        ],
      };
